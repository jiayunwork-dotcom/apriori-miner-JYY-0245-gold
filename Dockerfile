# apriori-miner —— 电商购物篮关联规则挖掘（Apriori）CLI
...
# 常用运行方式（在宿主机改、容器里跑）：
#   docker build -t apriori-miner .
#   docker run --rm -v "$PWD":/src apriori-miner go run . -input /src/example/transactions.txt -min-support 0.3 -min-confidence 0.6
